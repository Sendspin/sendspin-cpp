...
bool AnnouncementTask::start(bool task_stack_in_psram, unsigned priority) {
    ...
    this->event_flags_.clear(AnnouncementTaskBits::ANNOUNCEMENT_TASK_RUNNING |
                             ...
                             AnnouncementTaskBits::ANNOUNCEMENT_COMMAND_STREAM_DRAIN |
...
void AnnouncementTask::signal_stream_drain() {
    if (!this->is_initialized()) {
        return;
    }
    this->event_flags_.set(AnnouncementTaskBits::ANNOUNCEMENT_COMMAND_STREAM_DRAIN);
}
    ...
    while ((this->event_flags_.get() & AnnouncementTaskBits::ANNOUNCEMENT_COMMAND_STOP) == 0U) {
        ...
        while (true) {
            ...
            if ((flags & AnnouncementTaskBits::ANNOUNCEMENT_COMMAND_STREAM_END) != 0U) {
                // Discarded before playback started (transport loss): drop the buffer and abort.
                // A pending drain is superseded by the discard.
                this->event_flags_.clear(AnnouncementTaskBits::ANNOUNCEMENT_COMMAND_STREAM_END |
                                         AnnouncementTaskBits::ANNOUNCEMENT_COMMAND_STREAM_DRAIN);
                this->discard_ring_buffer();
                break;
            }
            if ((flags & AnnouncementTaskBits::ANNOUNCEMENT_COMMAND_START) != 0U) {
                // A drain that raced ahead of the start ack stays set and is honored by
                // play_stream(), which plays the buffered audio out before ending.
...
bool AnnouncementTask::wait_for_codec_header(AudioStreamInfo* stream_info) {
    while (true) {
        ...
        if ((flags & (AnnouncementTaskBits::ANNOUNCEMENT_COMMAND_STREAM_END |
                      AnnouncementTaskBits::ANNOUNCEMENT_COMMAND_STREAM_DRAIN)) != 0U) {
            // Stale end/drain command from a stream that ended while idle; clearing it here keeps
            // it from leaking into the next stream once a codec header arrives.
            this->event_flags_.clear(AnnouncementTaskBits::ANNOUNCEMENT_COMMAND_STREAM_END |
                                     AnnouncementTaskBits::ANNOUNCEMENT_COMMAND_STREAM_DRAIN);
        }
...
void AnnouncementTask::play_stream() {
    ...
    while (true) {
        ...
        if ((flags & AnnouncementTaskBits::ANNOUNCEMENT_COMMAND_STREAM_END) != 0U) {
            // Non-graceful end (transport loss): discard any remaining buffered audio and stop.
            this->event_flags_.clear(AnnouncementTaskBits::ANNOUNCEMENT_COMMAND_STREAM_END);
            this->discard_ring_buffer();
            break;
        }
        if ((flags & AnnouncementTaskBits::ANNOUNCEMENT_COMMAND_STREAM_DRAIN) != 0U) {
            // Graceful server stream/end: stop expecting new audio and play out what is buffered.
            // The loop keeps decoding and writing until the ring buffer empties (below), then ends.
            this->event_flags_.clear(AnnouncementTaskBits::ANNOUNCEMENT_COMMAND_STREAM_DRAIN);
            draining = true;
        }
        ...
        if (entry == nullptr) {
            if (draining) {
                break;  // Graceful end: the buffered tail has played out
            }
...
void AnnouncementTask::discard_ring_buffer() {
    while (true) {
