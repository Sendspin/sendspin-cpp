...
enum AnnouncementTaskBits : uint16_t {
    ...
    ANNOUNCEMENT_COMMAND_STREAM_END = (1 << 1),    // Discard the stream and end it (transport loss)
    ANNOUNCEMENT_COMMAND_STREAM_DRAIN = (1 << 2),  // Play out buffered audio, then end the stream
...
/// per-client, output begins at (or as soon as possible after) the stream's single
/// start_timestamp, and subsequent output is paced by the sink's blocking writes. A stalled
/// stream (buffer drained, no data arriving) is ended locally after a timeout.
///
...
public:
    ...
    /// @brief Signals the task to discard the current announcement stream and end it. Non-blocking
    /// The task drops any audio still in the ring buffer and returns to idle. Used for
    /// non-graceful teardown (transport loss), where the buffered tail cannot or should not play.
    /// Thread-safe: may be called from any context.
    void signal_stream_end();

    /// @brief Signals the task to stop expecting new audio and play out what remains. Non-blocking
    /// The task keeps decoding and writing the buffered audio until the ring buffer is empty, then
    /// ends the stream (which releases the ducking via OUTPUT_FINISHED). Used for a graceful
    /// server stream/end so the announcement tail is not cut off.
    /// Thread-safe: may be called from any context.
    void signal_stream_drain();
    ...
    /// @param timestamp The stream's start_timestamp when writing the codec header (the task
    ///                  reads the scheduled start time from it); 0 for the untimed audio chunks.
...
protected:
    ...
    /// Discards stale audio chunks. Captures the stream's start_timestamp (carried on the header
    /// chunk) into start_timestamp_. Returns true if a codec header was processed into the
    ...
    /// @brief Sleeps until the stream's start_timestamp (translated to the local clock), bounded
    ...
    /// @brief Non-blocking discard of all chunks currently in the ring buffer
    void discard_ring_buffer();
    ...
    // The stream's single scheduled start time (server clock, microseconds), captured from the
    // codec header chunk. Task-thread only. 0 means start as soon as possible.
