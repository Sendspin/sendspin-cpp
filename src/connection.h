...
public:
    ...
    /// @brief Sends a type-prefixed application binary message, encrypting it once Noise is active.
    /// @param data Pointer to the application payload (the role type byte is first).
    /// @param len Total payload length including the type byte.
    /// @param cb Optional completion callback. On the encrypted path it fires when encryption and
    ///           transport queuing finish; the transport owns any queued frame copy after return.
    /// @return SsErr::OK if sent/queued successfully, error code otherwise.
    SsErr send_app_binary(const uint8_t* data, size_t len, SendCompleteCallback cb = nullptr);
