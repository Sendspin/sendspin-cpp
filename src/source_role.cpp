...
void SourceRole::Impl::cleanup() {
    // Permission does not survive the connection (Sendspin spec, Source messages). Withdrawn
    // in one atomic transition before the synthetic STOPPED below: the listener can never
    // observe "stopped" while write_audio() still accepts, and a stream mid-open loses its
    // gate-open CAS rather than re-opening afterwards.
