...
- [ ] Standardize the API
- [x] ArtworkRole `on_image` fires directly from the network thread for received images but from the main thread for clears; either defer image data through a queue like other roles or document the split threading contract clearly
- [x] Wrap the `on_image` callback's 5 positional parameters in a struct (like `VisualizerFrame`)
