...
| `liveness_timeout_ms` | `int64_t` | `30000` | Milliseconds of inbound silence after which the established connection is dropped as dead and the listener sees the usual disconnect. Any complete inbound message counts, and a live server answers every time burst, so the default is two burst intervals plus one response timeout. Lower it together with the burst settings if you shorten those. `0` disables the check (a blackholed socket then stays "connected" forever). |
