...
| `reconnect_on_liveness_loss` | `bool` | `true` | Re-establish an outbound connection (`connect_to`) automatically after the liveness watchdog drops it as dead, retrying with exponential backoff (1 s doubling up to 30 s) until the server answers again. Inbound (discovery) connections are never reconnected: their server is expected to reconnect on its own. Disable if the integration drives reconnection itself. |
