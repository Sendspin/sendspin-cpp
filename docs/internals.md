...
`SourceRole::write_audio()` writes each call's bytes as one timestamped entry into a `SendspinAudioRingBuffer` (the same SPSC wrapper the player uses, outbound direction: producer = the consumer's capture thread, consumer = the source task). The path never waits on the consumer (zero ring timeout) and does not allocate; on host the ring's acquire/commit take a short mutex, on ESP it is lock-free. The `STREAM_ACCEPTING` bit of the task's `stream_state_` atomic gates it: set only between `client-stream/start` being sent and the stream closing, so writes outside an open stream are rejected without touching the ring. A full ring drops the write and warns once per overflow episode (the recovery log carries the drop total); the ring's capacity (`SourceRoleConfig::capture_buffer_ms`, sized with a +1/4 metadata margin — the inverse of the player's advertise fraction) is deliberately the stall-policy backlog bound.

### Task loop and stream binding

The task's outer loop idles on `SOURCE_COMMAND_STOP | SOURCE_COMMAND_UPDATE`. The desired streaming state is the `STREAM_DESIRED` bit of the same `stream_state_` atomic, set/cleared by the role's main-loop command latch via `signal_start()`/`signal_stop()`; the task converges on it, so a start-stop-start flurry cannot be misordered by event bits. Desired and accepting live in one atomic so opening the gate is a compare-exchange conditioned on the desired bit: a stop or cleanup that lands between the `client-stream/start` confirmation and the gate open wins the race, and `STREAMING_STARTED` is never emitted after the stop's synthetic STOPPED. When a stream is requested, the task binds it to ONE connection for its whole life (`ConnectionManager::current_shared()`): streaming permission is per-connection (Sendspin spec, Source messages), so a drop or handoff ends the stream rather than migrating it — `stream_still_open()` re-checks `current_shared() == conn` every iteration.
...
3. **Flush the ring and open the gate**: the flush makes the first chunk live audio by construction, and only then is the `STREAM_ACCEPTING` bit set — via a compare-exchange requiring `STREAM_DESIRED`, so a stop that raced the open leaves the gate shut and no STREAMING_STARTED is emitted. On a successful open a `SOURCE_STREAM` STREAMING_STARTED event is pushed onto the inbox ring.
...
5. **Close**: clear the `STREAM_ACCEPTING` bit first (so the tail is finite), send a final short chunk only if the encoder can take it (allowed at stream end by the spec but not required; an Opus remainder that is not a legal frame duration is dropped rather than padded), send `client-stream/end` from this same thread — ordered after the last chunk by construction — and push STREAMING_STOPPED.
...
Queued send workers capture a `weak_ptr<SendspinServerConnection>` to the originating connection — `AsyncRespArg` for text sends, `SessionLookup` for time sends, `BinarySendLookup` for binary sends — and `lock()` it when the worker runs. This is deliberately **not** a `{httpd_handle_t, int sockfd}` pair: identifying the target by sockfd risked binding to a *different* connection that had recycled the same fd after the original closed, sending a frame to the wrong peer. The `weak_ptr` resolves to the exact connection that queued the work, or to null if it has since been destroyed, in which case the worker no-ops cleanly.

`AsyncRespArg` and `SessionLookup` are allocated per message: because they hold non-trivial members (the `weak_ptr`, and `AsyncRespArg`'s completion `std::function`), they are constructed with placement-new and explicitly destroyed before `platform_free` rather than treated as POD, through `platform_malloc` / `platform_malloc_internal`. `BinarySendLookup` is different: one block per connection, created once with `std::make_shared` (a cold-path setup cost, off the per-chunk send path), backing the single-in-flight binary send slot. `async_send_binary` sends from the connection-owned payload buffer, releases the slot, and fires the completion exactly once on every exit path; the slot is otherwise released only by the connection destructor. Because `httpd_queue_work` has no cancellation hook, engaged blocks are tracked in an owner-scoped registry and reclaimed by `reclaim_orphaned_binary_send_work(handle)` after that server's clean `httpd_stop`, so a discarded worker's keep-alive cycle cannot strand a block and another live server's queued work is never touched.
