...
Encryption is mandatory, so the frame type no longer selects the handler: every application
...
1. Long-term record. `PskCategory::LONG_TERM`. Usually a record stored after a successful pairing, but also the shared-PSK fallback record that the constructor provisions before any pairing has happened: the unlocked `record_by_psk_id()` lookup behind this step does not distinguish the two, so both resolve as `LONG_TERM`. `resolve_pairing_outcome()` hands the shared record's PSK to a pairing server once the store is at capacity, so more than one server can hold it.
2. Accepted Pairing PSK (distributed out-of-band). `PskCategory::PAIRING`. Generated by `RecordStore`'s constructor when none is persisted (`pairing_psk` is the client-mandatory pairing method, so it must always have a key behind it) and surfaced to the operator by `SendspinClient::pairing_token()`.
3. Sentinel PSK: the published, well-known key, so it authenticates nothing on its own. `PskCategory::SENTINEL`. Distinct from the shared-PSK fallback record in step 1, which is device-generated and secret despite the similar name.
...
The long-term record is stored on the network thread (synchronously, inside `SendspinClient::process_json_message()`'s `SERVER_PAIR_FINALIZE` handler) so the immediately following `noise/handshake` msg1 can resolve the new `psk_id` from the record store. That handler calls `RecordStore::store_record_superseding()`, which retires any prior record for the same `server_id` before storing the new one. Like `store_record()`, it shares the `store_record_impl()` body: the persistence provider is written first and the call fails closed, so if the provider rejects the write the in-memory store is left untouched and the call returns `false`.
...
- **Persistence failure**: `RecordStore::store_record_superseding()` fails closed, so the record is never retained and `schedule_pairing_succeeded()` is skipped; a warning is logged. No dedicated abort path exists: the server rekeys onto that PSK regardless (it already acked pair-finalize), the client cannot resolve the `psk_id`, and the re-handshake drops the connection -- or, if the server never sends it, the re-armed 30 s provisional timeout does.
